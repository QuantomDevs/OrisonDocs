...
We are pleased to announce docs MDX 14. This update introduces several major enhancements designed to:
...
docs MDX now generates multiple entry files in `.source` folder.
...
// non-RSC environment
import browserCollections from 'quantomdocs-mdx:collections/browser';
```

To use `loader()` with doc collections, use the new `todocsSource()` utility instead.

```ts
import { blogPosts } from 'quantomdocs-mdx:collections/server';
import { todocsSource } from 'quantomdocs-mdx/runtime/server';
import { loader } from 'quantomdocs-core/source';

export const blog = loader({
  baseUrl: '/blog',
  source: todocsSource(blogPosts, []),
});
```
...
// or
import { postInstall } from 'quantomdocs-mdx/vite';
...
import { defineCollections, getDefaultMDXOptions } from 'quantomdocs-mdx/config';
...
import { defineCollections, applyMdxPreset } from 'quantomdocs-mdx/config';
import { myPlugin } from './remark-plugin';
...
Thank you for your continued support of docs. If you encounter any issues during the update, welcome to reach out through our GitHub Discussions.
