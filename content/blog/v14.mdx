---
title: docs v14
description: Introducing docs v14
author:  Nama
date: 2024-09-19
---
...
Taking some inspirations from [Shadcn UI](https://ui.shadcn.com), I decided to make docs easier to use, with more APIs that simplify the design.
...
Clone docs UI components to your workspace and modify them.

```package-install
npm install @docs/cli --save-dev
```
...
Previously, multiple page tree is implemented with docs UI `RootToggle` component.
You have to add it to the sidebar banner which isn't as intuitive as other APIs.

With Sidebar Tabs, by creating a root folder, docs will immediately add a tabs component to the sidebar.
...
import { createFromSource } from 'quantomdocs-core/search/server';

export const { GET } = createFromSource(source);
```

In favour of this, the new search client of docs is introduced with support for different API clients.
...
I'm always working to reduce the dependencies required for docs, that is one reason why we separated docs into different packages.

docs UI now bundles icons from `lucide-react`, for Next.js apps that is not using Lucide, they can avoid downloading the entire icon library.
...
To improve code organization, we made a Metadata Image API on docs Core.
It is a tiny wrapper over Next.js Metadata API, composes seamlessly with Source API.

```ts title="lib/metadata.ts"
import { createMetadataImage } from 'quantomdocs-core/server';
import { source } from '@/lib/source';
...
```ts title="route.ts"
import { generateOGImage } from 'quantomdocs-ui/og';
...
In favour of this, the `getImageMeta` function from `quantomdocs-ui/og` has been removed.

### Shorthands

As you may notice, we introduced more abstractions to reduce the setup steps required for enabling some features.
It is also required for the code generator from docs CLI to work.
...
docs UI Card can now support usage without `href`.
You can pass children as React node, Typography styles will be applied correctly.

```mdx
<Card icon={<Database />} title='Content Source'>

The input/source of your content, it can be a CMS, or local data layers like [Content Collections](https://www.content-collections.dev) and [docs MDX](/docs/mdx).
...
```tsx title="page.tsx"
import { source } from '@/lib/source';
import { DocsCategory } from 'quantomdocs-ui/page';
...
docs v14 is compatible with Next.js 15, supporting sync and async usages of dynamic APIs.
...
```ts
import { defineDocs } from 'quantomdocs-mdx/config';
...
```ts
import { DocsLayout } from 'quantomdocs-ui/layout';
import { HomeLayout } from 'quantomdocs-ui/home-layout';
import { HomeLayoutProps } from 'quantomdocs-ui/home-layout';
```
...
import { BaseLayoutProps } from 'quantomdocs-ui/layouts/shared';
```

### Twoslash UI

We moved Twoslash UI components to `docs-twoslash`.
This isolates some logic from docs UI, allowing a better code organization.

Import the CSS styles and `Popup` component differently.

```ts
import 'docs-twoslash/twoslash.css';

import { Popup } from 'docs-twoslash/ui';
```
...
The types from `quantomdocs-core/search/shared` is moved to `quantomdocs-core/server`.
