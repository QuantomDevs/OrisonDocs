...
docs v15 has no other significant changes other than migrating the config to Tailwind CSS v4.
...
Add `@import` to the presets exported by docs UI, and include the `quantomdocs-ui` package in the source.
...
@source '../node_modules/quantomdocs-ui/dist/**/*.js';
```
...
docs no longer use `--fd-<color>` like `--fd-primary: 0 0% 0%` for colors, it directly defines and uses colors in `@theme`.
...
v15 improves the playground UI (inspired by the minimalism of Scalar), and brought Scalar API Client support to docs OpenAPI.
...
import { APIPlayground } from 'quantomdocs-openapi/scalar';
...
<Callout>
  ...
  stylesheet of docs UI will conflict with their `style.css` because both
...
In the future, we also want to make further improvements to docs MDX:
