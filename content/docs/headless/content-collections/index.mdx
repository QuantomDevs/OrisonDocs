...
description: Use Content Collections for docs
---
...
To integrate with docs, add the following to your `content-collections.ts`.
...
import { transformMDX } from '@docs/content-collections/configuration';
...
To use components from other packages like docs UI, pass them to your `<MDXContent />` component.
...
<Callout title='Deep Dive: Why?'>
    ...
    To support importing a package from node modules, docs added a default value to the `cwd` option of MDX Bundler.
