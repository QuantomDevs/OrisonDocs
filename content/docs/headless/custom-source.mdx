...
**docs is very flexible.** You can integrate with any content source, even without an official adapter.

### Examples

You can see examples to use docs with a CMS, which allows a nice experience on publishing content, and real-time update without re-building the app.

- [BaseHub](https://github.com/-nama/docs-basehub)
- [Sanity](https://github.com/-nama/docs-sanity)
- [Community: Payload CMS by `MFarabi619`](https://github.com/MFarabi619/docs-payloadcms)
- [Community: Payload CMS by `bapspatil`](https://github.com/bapspatil/docs-payload-template)
...
```tsx
import { DocsPage, DocsBody } from 'quantomdocs-ui/layouts/docs/page';
...
docs offers a variant of [Search Integrations](/docs/headless/search).

## MDX Remote

docs offers the **MDX Remote** package, it is a helper to integrate Markdown-based content sources with docs.
You can think it as a `next-mdx-remote` with built-in plugins for docs.
...
```tsx
import { createCompiler } from '@docs/mdx-remote';
import { getPage } from './my-content-source';
import { DocsBody, DocsPage } from 'quantomdocs-ui/layouts/docs/page';
