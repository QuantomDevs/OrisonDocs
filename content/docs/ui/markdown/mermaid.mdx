...
docs doesn't have a built-in Mermaid wrapper provided, we recommend using `mermaid` directly.

## Setup

Install the required dependencies, `next-themes` is used with docs to manage the light/dark mode.
...
```tsx tab="docs MDX" title="source.config.ts"
import { remarkMdxMermaid } from 'quantomdocs-core/mdx-plugins';
