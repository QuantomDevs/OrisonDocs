---
title: Themes
description: Add Theme to docs UI
---
...
docs UI adds its own colors, animations, and utilities with Tailwind CSS preset.

### Setup

Only Tailwind CSS v4 is supported, the preset will also include source to docs UI itself:

```css title="Tailwind CSS"
@import 'tailwindcss';
@import 'quantomdocs-ui/css/neutral.css';
@import 'quantomdocs-ui/css/preset.css';
```

For **Shadcn UI**, you can use the `shadcn` preset instead. docs UI will adopt colors from your Shadcn UI theme.

```css
@import 'tailwindcss';
@import 'quantomdocs-ui/css/shadcn.css';
@import 'quantomdocs-ui/css/preset.css';
```
...
docs supports light/dark modes with [`next-themes`](https://github.com/pacocoursey/next-themes), it is included in Root Provider.
...
```tsx
import { RootProvider } from 'quantomdocs-ui/provider/<framework>';
...
```css
@import 'quantomdocs-ui/css/<theme>.css';
@import 'quantomdocs-ui/css/preset.css';
```
...
> The plugin is supposed to work with docs UI's MDX components, it may conflict with `@tailwindcss/typography`.
>
