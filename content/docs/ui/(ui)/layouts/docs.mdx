...
A mobile-only navbar, the `nav` prop is also reusable on other docs layouts.
...
```tsx title="layout.tsx"
import { baseOptions } from '@/lib/layout.shared';
import { DocsLayout } from 'quantomdocs-ui/layouts/notebook';
import type { ReactNode } from 'react';
...
docs uses **CSS Variables** to share the size of layout components, and fit each layout component into appropriate position.
