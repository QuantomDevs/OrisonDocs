...
You can run `quantomdocs-mdx` to generate types & entry files (the `.source` folder).
...
```npm
npx quantomdocs-mdx "config-path" "output-dir"
```

- `config-path`: docs MDX config file, default to `source.config.ts`.
