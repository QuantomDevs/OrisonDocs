...
description: Built-in document search of docs
---

docs supports document search with Orama, It is the default but also the recommended option since it can be self-hosted and totally free.
...
Usually, it is provided by your content source (e.g. docs MDX). You can also extract it from Markdown/MDX document using the [Remark Structure](/docs/headless/mdx/structure) plugin.

```ts tab="Next.js" title="app/api/search/route.ts"
import { source } from '@/lib/source';
import { createSearchAPI } from 'quantomdocs-core/search/server';
...
- **docs UI**: Supported out-of-the-box, see [Search UI](/docs/ui/search/orama) for details.
- **Search Client**:
...
<auto-type-table type='Extract<import("quantomdocs-core/search/client").Client, { type: "fetch" }>' />
...
- **docs UI**: Configure [Tag Filter](/docs/ui/search/orama#tag-filter) on Search UI.
...
- **docs UI**: use [static client](/docs/ui/search/orama#static) on Search UI.
...
- **Search Client**: use `static` instead of `fetch`.
  ...
  <auto-type-table type='Extract<import("quantomdocs-core/search/client").Client, { type: "static" }>' />
...
import { source } from '@/lib/source';
import { createI18nSearchAPI } from 'quantomdocs-core/search/server';
...
- **docs UI**: No changes needed, docs UI handles this when you have i18n configured correctly.
- **Search Client**:
...
```ts
import { initAdvancedSearch } from 'quantomdocs-core/search/server';
