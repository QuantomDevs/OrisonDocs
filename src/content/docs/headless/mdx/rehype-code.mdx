...
A wrapper of [`@shikijs/rehype`](https://shiki.style/packages/rehype), the syntax highlighter used by docs.
...
import { rehypeCode, type RehypeCodeOptions } from 'quantomdocs-core/mdx-plugins';
...
export default defineConfig({
  mdxOptions: {
    rehypeCodeOptions: {
      // enabled by default on docs MDX
...
```ts
import { type RehypeCodeOptions } from 'quantomdocs-core/mdx-plugins';
