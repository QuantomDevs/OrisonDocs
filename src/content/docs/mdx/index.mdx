...
description: Introducing docs MDX, the official content source of docs.
icon: Album
---

## Introduction

docs MDX is a tool to transform content into type-safe data, similar to Content Collections.
...
docs MDX transforms collections into type-safe data, accessible in your app. Available collections:
...
import { defineCollections } from 'quantomdocs-mdx/config';
...
Combination of `meta` and `doc` collections, which is needed for docs.
...
docs MDX uses [MDX Compiler](https://mdxjs.com/packages/mdx) to compile MDX files into JavaScript files.
