...
```bash
pip install ./node_modules/docs-python
```

Generate the docs as a JSON:

```bash
py-generate your-package-name
# for example
py-generate httpx
```
...
import * as Python from 'docs-python';
...
<Callout type="warn" title="Be careful">
  While most docgens use Markdown or reStructuredText, docs uses **MDX**.
...
import * as Python from 'docs-python/components';
