...
docs provides many useful extensions to MDX, a markup language. Here is a brief introduction to the default MDX syntax of docs.

> MDX is not the only supported format of docs. In fact, you can use any renderers such as headless CMS.
...
docs support YAML-based frontmatter by default, `title` represents the page title (`h1`) in docs UI.
...
```ts tab="docs MDX" title="source.config.ts"
...
import { remarkCodeTab } from 'quantomdocs-core/mdx-plugins';
...
> This is only available on **docs MDX**.
...
When using docs MDX, you can customise it like:
