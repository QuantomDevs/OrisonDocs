---
title: The early mistake of docs
...
The early mistake of docs was **docs CLI being too late and not getting enough mentions on docs**.
...
- Advanced customisations can use docs CLI, avoiding upstream changes.
...
Page tree is actually a crucial idea of docs, unifying all navigation & linking of docs.
...
docs is using CSS variables for all colors since the beginning, the naming is identical to Shadcn UI which makes customisation easy.
...
- **Continue improving docs CLI:** adding install-time optimization to remove unused files according to your options (e.g. use existing sidebar component instead of installing one).
- **A better versioning policy:** Reduce abstraction for mitigating upstream changes, instead version the components directly (e.g. creating a new `quantomdocs-ui/layouts/v2/page` instead of a major UI change).
- **Add docs-native cloud features:** More official integrations like monitoring/analytics feature designed for docs.
