...
```ts
import createMDX from 'quantomdocs-mdx/config';
...
Now you can generate types using `quantomdocs-mdx` command, it's recommended to set it as a `postinstall` script.

```json title="package.json"
{
  "scripts": {
    "postinstall": "quantomdocs-mdx"
  }
}
```
...
Now to integrate it with docs framework, change your `source.ts` from:

```ts
import { map } from '@/.map';
import { createMDXSource } from 'quantomdocs-mdx/runtime/next';
...
```ts
import { createMDXSource } from 'quantomdocs-mdx/runtime/next';
import type { InferMetaType, InferPageType } from 'quantomdocs-core/source';
import { loader } from 'quantomdocs-core/source';
...
You can read the latest docs of docs MDX for details.

<Cards>
  <Card href="/docs/mdx" title="docs MDX">
